...
{
    Property *parentItem = m_rootItem;
    if (parent.isValid()) parentItem = static_cast<Property*>(parent.internalPointer());
...
void QPropertyModel::addItem( QObject* propertyObject )
{
    ...
    for( const QMetaObject* obj : classList )// remove empty classes from hierarchy list
    {
        bool keep = false;
    ...
    for( const QMetaObject* metaObject : finalClassList )
    {
        for( PropertyPair pair : propertyMap )
        {
            if( pair.Object == metaObject )// Check if the property is associated with the current class from the finalClassList
            {
