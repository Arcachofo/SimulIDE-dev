...
void Circuit::loadStrDoc( QString &doc )
{
    ...
    m_conList.unite( conList );

    if( m_pasting && !compList.isEmpty() )  {
        beginBatchProcess();

        // clear all state generated by copy component properties
        addCompState( NULL, "New State", stateNew);

        // save states
        for( Component *comp : compList ) addCompState( comp, COMP_STATE_CREATED, stateAdd);
        for( Node *node : nodeList ) addCompState( node, COMP_STATE_CREATED, stateAdd);
        for( Connector *con : conList ) addCompState( con, COMP_STATE_CREATED, stateAdd);
...
{
    for( Connector *con : m_removedConnectors ) delete con;
    for( Component *comp : m_removedComps ) delete comp;
...
void Circuit::removeItems()                     // Remove Selected items
{
    ...
    beginBatchProcess();
    QSet<Connector*> oldConnectors = m_conList;
    QSet<Component*> oldComponents = m_compList;
    QSet<Node*> oldNodes = m_nodeList;

    //save all comps
    QMap<CompBase *,QString> compStrMap;
    for( Connector* con  : oldConnectors ) compStrMap.insert( con, con->toString() );
    for( Node* node : oldNodes ) compStrMap.insert( node, node->toString() );
    for( Component* comp : oldComponents ) compStrMap.insert( comp, comp->toString() );
    ...
    for( Connector* con : conns ) {
        if ( m_conList.contains( con ) ) removeConnector( con );
    }
    for( Component* comp: comps ) {
        if ( m_compList.contains( comp ) ) {
            if ( comp->itemType() == "Node" ) {
                Node * node = dynamic_cast<Node *>(comp);
                if ( m_nodeList.contains( node ) ) removeNode( node );
            } else {
                if ( m_compList.contains( comp ) ) removeComp( comp );
            }
        }
    }
    addCompState( NULL, "New State", stateNew );
    for( Connector* con  : oldConnectors )  {
        if ( !m_conList.contains( con ) ) addCompRemovedState( con->getUid(), compStrMap.value(con) );
    }
...
void Circuit::removeConnector( Connector *conn )
{
    ...
    conn->remove();
    m_conList.remove( conn );
    m_removedConnectors.insert( conn );
}
...
void Circuit::addCompState( CompBase* c, QString p, stateMode state )
{
    ...
    if ( c )
        qDebug() << "Circuit::addCompState" << c->getUid() << p << state;
    else
...
bool Circuit::restoreState( circState step )
{
    ...
    if( m_undo || m_redo ) addCompState( NULL, "", stateNew ); // Start new Undo/Redo state

    int iStep, i;
    if( m_undo ) {
        iStep = -1; i = step.compStates.size() - 1 ;
    } else {
        iStep = 1; i = 0;
    }
...
void Circuit::newconnector( Pin* startpin )
{
    ...
    m_conList.insert( m_newConnector );
