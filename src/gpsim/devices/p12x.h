...
class P12_OSCCON : public  SfrReg
{
public:
    enum {
        FOSC4 = 1 << 0
    ...
    P12_OSCCON(Processor *pCpu, const char *pName )
        : SfrReg( pCpu,pName ), m_CPU(0)
    { }
...
{
public:
    GPIO(P12bitBase *pCpu, const char *pName,
         uint numIopins,
         uint enableMask,
         uint resetMask  = (1 << 3),
...
public:
    ...
    virtual uint fsr_valid_bits() { return 0x3f; } // 64 registers in all (some are actually aliased)


    virtual uint fsr_register_page_bits() { return 0x20; } // 519 has 2 register banks


    P12CE519(const char *_name=0  );
    ~P12CE519();
    static Processor *construct(const char *name);
    virtual void create();
};
