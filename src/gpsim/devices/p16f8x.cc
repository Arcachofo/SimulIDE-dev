...
public:
    Config1(pic_processor *pCpu)
        : ConfigWord("CONFIG1", 0x3fff, pCpu, 0x2007)
    {
    }
...
P16F8x::P16F8x(const char *_name )
...
void P16F8x::create_iopin_map()
{
    ...
    assign_pin( 4, m_porta->addPin(new IO_bi_directional("porta5"),5));
...
void P16F8x::create_sfr_map()
{
    ...
    comparator.initialize(get_pir_set(), &(*m_porta)[2], &(*m_porta)[0],
            &(*m_porta)[1], &(*m_porta)[2], &(*m_porta)[3], &(*m_porta)[3], &(*m_porta)[4]);
    ...
    add_SfrReg(&comparator.cmcon, 0x9c, RegisterValue(7,0),"cmcon");
    add_SfrReg(&comparator.vrcon, 0x9d, RegisterValue(0,0),"cvrcon");
...
void  P16F8x::create(int eesize)
{
    ...
    osccon = new OSCCON_1(this, "osccon" );
...
P16F81x::P16F81x(const char *_name )
    : P16X6X_processor(_name ),
      adcon0(this,"adcon0" ),
      adcon1(this,"adcon1" ),
      adresh(this,"adresh" ),
...
bool P16F81x::set_config_word(uint address, uint cfg_word)
{
    ...
    if (pic_processor::set_config_word(address, cfg_word))
    {
        ...
        // If the /MCLRE bit is set then RA5 is the MCLR pin, otherwise it's
        // a general purpose I/O pin.
        if ((cfg_word & CFG_MCLRE)) assignMCLRPin(4);
        else                        unassignMCLRPin();

        if (cfg_word & CFG_CCPMX) ccp1con.setIOpin(&((*m_portb)[2]));
        else                      ccp1con.setIOpin(&((*m_portb)[3]));
...
P16F87::P16F87(const char *_name )
    : P16F8x(_name )
{
...
P16F88::P16F88(const char *_name )
    : P16F87(_name ),
      ansel(this,"ansel" ),
      adcon0(this,"adcon0" ),
      adcon1(this,"adcon1" ),
...
void P16F88::create_sfr_map()
{
    ...
    add_SfrReg(&adcon0, 0x1f, RegisterValue(0,0));
    add_SfrReg(&adcon1, 0x9f, RegisterValue(0,0));
    add_SfrReg(&ansel, 0x9b, RegisterValue(0x7f,0));
...
P16F818::P16F818(const char *_name )
    : P16F81x(_name )
{
}
...
P16F819::P16F819(const char *_name )
    : P16F81x(_name )
{
}
