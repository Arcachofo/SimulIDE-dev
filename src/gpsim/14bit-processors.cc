...
public:
    ...
    virtual void set(int64_t v)
    {
        ...
        if (m_pCpu)
        {
            ...
            if( diff & WDTEN ) m_pCpu->wdt.initialize ((v & WDTEN) == WDTEN);
        }
    }
...
//-------------------------------------------------------------------
_14bit_processor::_14bit_processor(const char *_name  )
    : pic_processor(_name  ), intcon(0),
...
Pic14Bit::Pic14Bit(const char *_name )
...
void Pic14Bit::create_sfr_map()
{
    ...
    //add_SfrReg(&intcon_reg, 0x8b, RegisterValue(0,0));
...
_14bit_e_processor::_14bit_e_processor(const char *_name )
    : _14bit_processor(_name ),
      mclr_pin(4),
      intcon_reg(this,"intcon" ),
      option_reg(this,"option_reg" ),
      bsr(this, "bsr" ),
      pcon(this, "pcon" , 0xcf),
      wdtcon(this, "wdtcon" , 0x3f),
      ind0(this,string("0")),
      ind1(this,string("1")),
      status_shad(this, "status_shad" ),
      wreg_shad(this, "wreg_shad" ),
      bsr_shad(this, "bsr_shad" ),
      pclath_shad(this, "pclath_shad" ),
      fsr0l_shad(this, "fsr0l_shad" ),
      fsr0h_shad(this, "fsr0h_shad" ),
      fsr1l_shad(this, "fsr1l_shad" ),
      fsr1h_shad(this, "fsr1h_shad" )
{
    delete stack;
...
_14bit_e_processor::~_14bit_e_processor()
{
    ...
    remove_SfrReg(&status_shad);
    remove_SfrReg(&wreg_shad);
    remove_SfrReg(&bsr_shad);
    remove_SfrReg(&pclath_shad);
    remove_SfrReg(&fsr0l_shad);
    remove_SfrReg(&fsr0h_shad);
    remove_SfrReg(&fsr1l_shad);
    remove_SfrReg(&fsr1h_shad);

    Stack14E *stack14E = static_cast<Stack14E *>(stack);
    remove_SfrReg(&stack14E->stkptr);
    remove_SfrReg(&stack14E->tosl);
    remove_SfrReg(&stack14E->tosh);
}
...
void _14bit_e_processor::create_sfr_map()
{
    ...
    add_SfrReg(&ind0.indf,  0x00, RegisterValue(0,0), "indf0");
    add_SfrReg(&ind1.indf,  0x01, RegisterValue(0,0), "indf1");

    add_SfrReg(pcl,     0x02, RegisterValue(0,0));
    add_SfrReg(status,  0x03, RegisterValue(0x18,0));
    add_SfrReg(&ind0.fsrl,  0x04, RegisterValue(0,0), "fsr0l");
    add_SfrRegR(&ind0.fsrh,  0x05, RegisterValue(0,0), "fsr0h");
    add_SfrReg(&ind1.fsrl,  0x06, RegisterValue(0,0), "fsr1l");
    add_SfrRegR(&ind1.fsrh,  0x07, RegisterValue(0,0), "fsr1h");
    add_SfrReg(&bsr,     0x08);
    add_SfrReg(Wreg,     0x09);
    add_SfrReg(pclath,  0x0a, RegisterValue(0,0));
    add_SfrRegR(&intcon_reg, 0x0b, RegisterValue(0,0));

    add_SfrReg(&pcon,   0x96, RegisterValue(0x0c,0),"pcon");
    wdt.set_postscale(0);
    wdt.set_timeout(1./32000.);
    add_SfrRegR(&wdtcon,   0x97, RegisterValue(0x16,0),"wdtcon");


    // These are copies taken at an interrupt
    add_SfrReg(&status_shad,  0xfe4);
    add_SfrReg(&wreg_shad,    0xfe5);
    add_SfrReg(&bsr_shad,     0xfe6);
    add_SfrReg(&pclath_shad,  0xfe7);
    add_SfrReg(&fsr0l_shad,   0xfe8);
    add_SfrReg(&fsr0h_shad,   0xfe9);
    add_SfrReg(&fsr1l_shad,   0xfea);
    add_SfrReg(&fsr1h_shad,   0xfeb);

    Stack14E *stack14E = static_cast<Stack14E *>(stack);
    add_SfrReg(&stack14E->stkptr,  0xfed,RegisterValue(0,0),"stkptr");
    add_SfrReg(&stack14E->tosl,    0xfee,RegisterValue(0,0),"tosl");
    add_SfrReg(&stack14E->tosh,    0xfef,RegisterValue(0,0),"tosh");
...
public:
    Config_E(_14bit_e_processor *pCpu, const char *name, uint address, bool EEw=false)
        : ConfigWord(name, 0x3fff, pCpu, address, EEw)
    {
        if (m_pCpu)
        {
