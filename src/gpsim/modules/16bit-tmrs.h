...
public:
    ...
    T5CON(Processor *pCpu, const char *pName );
    ~T5CON();
    ...
    virtual uint get_prescale() { return(((value.get() &(TxCKPS0 | TxCKPS1)) >> 4) ); }

    virtual uint get_tmr1cs() { return((value.get() & (TMRxCS0 | TMRxCS1)) >> 6); }
    virtual bool get_tmr1on() { return(value.get() & TMRxON); }
    virtual bool  get_t1oscen() { return(value.get() & TxSOSCEN); }
    virtual bool get_tmr1GE() { return t1gcon->get_tmr1GE(); }
    virtual bool get_t1sync() { return(value.get() & TxSYNC); }
...
class CCPTMRS0 : public SfrReg
{
public:
    CCPTMRS0(CCPTMRS *_ccptmrs,  Processor *pCpu, const char *pName );
    ~CCPTMRS0();
...
class CCPTMRS1 : public SfrReg
{
public:
    CCPTMRS1(CCPTMRS * _ccptmrs, Processor *pCpu, const char *pName );
    ~CCPTMRS1();
