...
void SPI::clock( bool ClockState )
{
    ...
    if( m_state == eIDLE ){
        ...
        {
            // FIX: I have NOT verified that PICs actually behave like this.
            cout << "SSP: " << cpu->name_str << " Ignoring clock transition to neutral in state IDLE." << endl;
            return;
        }
        else
        {
...
void SPI::set_halfclock_break()
{
    ...
    cpu->setBreakRel( clock_in_cycles, this );
}
void SPI::callback()
{
    if (!m_sspmod)
...
void I2C::callback()
{
    if( future_cycle != cpu->currentCycle() )
    {
        cout << "I2C callback - program error future_cycle=" << future_cycle << " now="
             << cpu->currentCycle() << " i2c_state=" << i2c_state << endl;
    }
...
void I2C::setBRG()
{
    ...
    future_cycle = cpu->currentCycle() + ((m_sspadd->get() &0x7f)/ 4) + 1;
    ...
    if (m_sspmod->get_SCL_State())        // Clock is high
    {
        ...
        else                // Data going low - START
        {
            ...
            case CLK_START:
                if (phase == 0)
                {
                    uint64_t fc = cpu->currentCycle() + ((m_sspadd->get() &0x7f)/ 2) + 1;
