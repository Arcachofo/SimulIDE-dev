...
//--------------------------------------------------
//
FSRL::FSRL(Processor *pCpu, const char *pName,Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName ),
      iam(pIAM)
{
}
...
FSRH::FSRH(Processor *pCpu, const char *pName, Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName),
      iam(pIAM)
{
}
...
INDF16::INDF16(Processor *pCpu, const char *pName, Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName ),
      iam(pIAM)
{
}
...
//------------------------------------------------
// PREINC
PREINC::PREINC(Processor *pCpu, const char *pName, Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName ),
      iam(pIAM)
{
}
...
//------------------------------------------------
// POSTINC
POSTINC::POSTINC(Processor *pCpu, const char *pName, Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName ),
      iam(pIAM)
{
}
...
//------------------------------------------------
// POSTDEC
POSTDEC::POSTDEC(Processor *pCpu, const char *pName, Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName ),
      iam(pIAM)
{
}
...
//------------------------------------------------
// PLUSW
PLUSW::PLUSW(Processor *pCpu, const char *pName, Indirect_Addressing *pIAM)
    : SfrReg(pCpu,pName ),
      iam(pIAM)
{
}
...
{
    int destination = iam->plusw_fsr_value();
    if(destination >= 0) cpu_pic->registers[destination]->put(new_value);
}

void PLUSW::put_value(uint new_value)
{
    int destination = iam->plusw_fsr_value();
    if(destination >= 0)  cpu_pic->registers[destination]->put_value(new_value);

    update();
    if(destination >= 0) cpu_pic->registers[destination]->update();
}
...
Indirect_Addressing::Indirect_Addressing(pic_processor *pCpu, const string &n)
    : fsrl(pCpu,( string("fsrl")+n).c_str(), this),
      ...
      preinc(pCpu,( string("preinc")+n).c_str(), this),
      postinc(pCpu,( string("postinc")+n).c_str(), this),
      postdec(pCpu,( string("postdec")+n).c_str(), this),
      plusw(pCpu,( string("plusw")+n).c_str(), this)
{
...
{
    if( is_indirect_register(fsr_value) ) return;
...
uint Indirect_Addressing::get()
{
    ...
    if(is_indirect_register(fsr_value))
         return 0;
    else return cpu_pic->registers[get_fsr_value()]->get();
}
...
void Indirect_Addressing::preinc_fsr_value()
{
    if(current_cycle != cpu->currentCycle())
    {
        ...
        fsr_delta = 0;
        current_cycle = cpu->currentCycle();
        put_fsr(fsr_value);
    }
}
...
//--------------------------------------------------
PCL16::PCL16(Processor *pCpu, const char *pName )
    : PCL(pCpu,pName )
{
}
...
//------------------------------------------------
// TOSU
TOSU::TOSU(Processor *pCpu, const char *pName )
    : SfrReg(pCpu,pName )
{}
uint TOSU::get()
{
...
//------------------------------------------------
// STKPTR
STKPTR16::STKPTR16(Processor *pCpu, const char *pName )
    : SfrReg(pCpu,pName )
{}
...
//--------------------------------------------------
RCON::RCON(Processor *pCpu, const char *pName )
    : SfrReg(pCpu,pName )
{
}
//--------------------------------------------------
...
//--------------------------------------------------
CPUSTA::CPUSTA(Processor *pCpu, const char *pName )
    : SfrReg(pCpu,pName )
{
}
//--------------------------------------------------
...
//--------------------------------------------------
T0CON::T0CON(Processor *pCpu, const char *pName )
...
void T0CON::put(uint new_value)
{
    ...
    if(value.get() & TMR0ON) cpu16->tmr0l.start(initialTmr0value);
    else                     cpu16->tmr0l.stop();
}
...
TMR0H::TMR0H(Processor *pCpu, const char *pName )
    :SfrReg(pCpu,pName )
{
}
...
//
TMR0_16::TMR0_16(Processor *pCpu, const char *pName )
    : TMR0(pCpu,pName ),
...
uint TMR0_16::get_prescale()
{
    if(t0con->value.get() & 0x8)
         return 0;
    else return( (t0con->value.get() & 7) + 1);
}
...
void TMR0_16::put_value(uint new_value)
{
    ...
    {
        if(t0con->value.get() & T0CON::T08BIT) TMR0::put_value(new_value);
        else start(value16);
    } else {
        // TMR0 is not enabled
    }
    ...
    if(--prescale_counter == 0 )
    {
        ...
        else
        {
            if( value.get() == 255 )
            {
                ...
                else tmr0h->value.put(tmr0h->value.get()+1);
            }
            else value.put(value.get()+1);
        }
    }
...
uint TMR0_16::get_value()
{
    if(t0con->value.get() & T0CON::TMR0ON)
    {
        ...
        if(t0con->value.get() & T0CON::T08BIT)
        {
            if( tmr0h) tmr0h->put_value(( value16>>8)&0xff);

            return(TMR0::get_value());
        }
        value16 =( int)( (cpu->currentCycle() - last_cycle)/ prescale);
...
uint TMR0_16::get()
{
    ...
    if( t0con->value.get() & T0CON::T08BIT ) return value.get();
...
void TMR0_16::callback()
{
    ...
    if(future_cycle && !(t0con->value.get() & T0CON::T08BIT))
    {
        tmr0h->put_value(0); // 16-bit mode
    }
}

void TMR0_16::callback_print()
{
    cout << "TMR0_16 " << name_str << " CallBack ID " << CallBackID << '\n';
}
...
//--------------------------------------------------
// T3CON
T3CON::T3CON(Processor *pCpu, const char *pName )
    : T1CON(pCpu,pName ),
...
void T3CON::put(uint new_value)
{
    ...
    T1CON::put(new_value  & ~(T3CCP1 |  T3CCP2)); // Let the T1CON class deal with everything else.
}
...
void TBL_MODULE::increment()
{
    ...
    if(tblptrl.value.get() >= 0xff)
    {
        ...
        else tblptrh.put(tblptrh.value.get() + 1);
    }
    else tblptrl.put(tblptrl.value.get() + 1);
...
void TBL_MODULE::read()
{
    ...
    opcode = cpu_pic->get_program_memory_at_address(tblptr & 0xfffffe);
...
void TBL_MODULE::write()
{
    ...
    if(tblptr & 1) *pt =( *pt & 0x00ff) |( (tablat.value.get()<<8) & 0xff00);
    else           *pt =( *pt & 0xff00) |( tablat.value.get() & 0x00ff);
...
void TBL_MODULE::start_write()
{
    ...
    if( eecon1.value.get() &( EECON1::EEPGD|EECON1::CFGS))
    {
        ...
        cpu->setBreakRel(( uint64_t)(.002*cpu->instruction_cps()), this);
        cpu->set_pm_write();
        cpu_pic->pm_write();
    }
    else
    {
        cpu->setBreakRel( EPROM_WRITE_TIME, this );
...
//-------------------------------------------------------------------
//-------------------------------------------------------------------
LVDCON::LVDCON(Processor *pCpu, const char *pName )
    : SfrReg(pCpu, pName ),
      valid_bits(0x3f)
{
}
...
HLVDCON::HLVDCON(Processor *pCpu, const char *pName ) :
    SfrReg(pCpu, pName), hlvdin(0),
     ////hlvdin_stimulus(0),
...
HLVDCON::~HLVDCON()
{
    ...
    if( stimulus_active )
    {
     ...
     ////if( hlvdin_stimulus) delete hlvdin_stimulus;
}
...
void HLVDCON::put(uint new_value)
{
    ...
    if( diff & HLVDEN )
    {
        if( new_value & HLVDEN )	// Turning on
        {
            ...
            cpu->setBreakRel( tivrst*cpu->instruction_cps(), this );
            return;
        }
        else	// Turning off
        {
    ...
    if( !(value.get() & IRVST))	return;// Just return if voltage not stable
...
void HLVDCON::check_hlvd()
{
    ...
    if( (reg & HLVDL_MASK) == HLVDL_MASK)	// using HLVDIN pin
    {
        ...
        if      ( (reg & VDIRMAG) &&( voltage >= 1.024) ) IntSrc->Trigger(); // High voltage trip
        else if( !(reg & VDIRMAG) &&( voltage <= 1.024) ) IntSrc->Trigger(); // Low voltage trip
    }
    else	// Voltage divider on Vdd
    {
        double voltage = hldv_volts[reg & HLVDL_MASK];

        if      (( reg & VDIRMAG) &&( cpu->get_Vdd() >= voltage) ) IntSrc->Trigger();
        else if( !(reg & VDIRMAG) &&( cpu->get_Vdd() <= voltage) ) IntSrc->Trigger();
    }
}
