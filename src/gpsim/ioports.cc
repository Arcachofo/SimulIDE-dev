...
// getState is called when the PinModule is attempting to update the output state for the I/O Pin.
...
//------------------------------------------------------------------------
PortRegister::PortRegister( Processor* pCpu, const char *pName, uint numIopins, uint _mask)
    : SfrReg( pCpu, pName   ),
...
void PortRegister::setbit(uint bit_number, char new3State)
{
    ...
    if( set_mask & mValidBits )
    {
        ...
        else rvDrivenValue.init |= (1<<bit_number);// Not a 0 or 1, so it must be unknown.
    ...
    : mNumIopins(numIopins)
{
    ...
    for( uint i=0; i<mNumIopins; i++ ) iopins[i] = &AnInvalidPinModule;
}

PortModule::~PortModule()
{
    for( uint i=0; i<mNumIopins; i++ )
        if( iopins[i] != &AnInvalidPinModule ) delete iopins[i];
...
void PortModule::addSink( SignalSink* new_sink, uint iPinNumber )
{
    if( iPinNumber < mNumIopins ) iopins[iPinNumber]->addSink( new_sink );
}
...
IOPIN* PortModule::addPin( IOPIN* new_pin, uint iPinNumber )
{
    ...
    {
        if( iopins[iPinNumber] == &AnInvalidPinModule) // If there is not a PinModule for this pin, then add one.
            iopins[iPinNumber] = new PinModule(this,iPinNumber);

        iopins[iPinNumber]->setPin(new_pin);
    }
    else printf( "PortModule::addPin ERROR pin %u > %u\n", iPinNumber, mNumIopins );
...
{
    for( int i=0; i<ANALOG_TABLE_SIZE; i++) m_analog_reg[i] = NULL;
}
...
void PinModule::setDrivenState( char new3State )
{
    ...
    for( ssi=sinks.begin(); ssi!=sinks.end(); ++ssi ) (*ssi)->setSinkState( new3State );
}
...
void PinModule::AnalogReq( Register* reg, bool analog, const char* newname )
{
    ...
    if( index < 0 ) // Register is not in table so add it.
    {
