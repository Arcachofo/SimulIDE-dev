...
static avr_cycle_count_t avr_timer_tov( struct avr_t* avr, avr_cycle_count_t when, void* param )
{
    ...
    for( int compi=0; compi<AVR_TIMER_COMP_COUNT; compi++ )
    {
        ...
        if( comp_cycles )
        {
            if( p->wgm_kind == wgm_fc_pwm )
            {
                if( p->countUp ) // Counting Up
                {
                    next_match = p->tov_cycles-next_match;
                }
...
static uint8_t avr_timer_tcnt_read(
        struct avr_t * avr,
        avr_io_addr_t addr,
        void * param)
{
...
static void avr_timer_write_ocr( struct avr_t* avr, avr_io_addr_t addr, uint8_t v, void* param )
{
    ...
    if( (comp_n == 0) && (timer->Wgm_mode.top == wgm_reg_ocra) ) avr_timer_reconfigure( timer, 0 );
    else
    {
        static const avr_cycle_timer_t dispatch[AVR_TIMER_COMP_COUNT] =
            { avr_timer_compa, avr_timer_compb, avr_timer_compc };
    ...
    switch( timer->wgm_kind )
    {
        ...
        case wgm_fc_pwm:
            ...
            else                               // if OCRA or Max is the top, reconfigure needed
            {
                avr_timer_reconfigure( timer, 0 );
            }
            break;
