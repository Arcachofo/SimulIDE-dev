...
static uint8_t avr_acomp_get_state( struct avr_t* avr, avr_acomp_t* ac)
{
    if( avr_regbit_get(avr, ac->disabled) ) return 0;

    uint16_t positive_v; // get positive voltage

    if( avr_regbit_get(avr, ac->acbg) ) positive_v = ACOMP_BANDGAP;// if bandgap
    else                                positive_v = ac->ain_values[0];    // AIN0

    uint16_t negative_v = 0;  // get negative voltage
    ...
    else negative_v = ac->ain_values[1];    // AIN1

    return positive_v > negative_v;
}

static avr_cycle_count_t avr_acomp_sync_state( struct avr_t* avr,
...
static inline void avr_schedule_sync_state( struct avr_t* avr, void* param)
{
    avr_cycle_timer_register( avr, 1, avr_acomp_sync_state, param );
}

static void avr_acomp_write_acsr(
    struct avr_t* avr, avr_io_addr_t addr,
    uint8_t v, void* param)
{
    avr_acomp_t* p = (avr_acomp_t*)param;

    uint8_t aciBit = 1<<p->ac.raised.bit;
    uint8_t old_aci = avr->data[addr] & aciBit;
    uint8_t new_aci = v & aciBit;

    if( old_aci && !new_aci ) v |= aciBit;
    else                      v &= ~aciBit;
...
static void avr_acomp_dependencies_changed( struct avr_irq_t* irq, uint32_t value, void* param)
{
    avr_acomp_t* p = (avr_acomp_t*)param;
    avr_schedule_sync_state( p->io.avr, param);
}

static void avr_acomp_irq_notify( struct avr_irq_t* irq, uint32_t value, void* param )
{
    avr_acomp_t* p = (avr_acomp_t*)param;
...
static void avr_acomp_register_dependencies( avr_acomp_t* p, avr_regbit_t rb)
{
    if (rb.reg)
    {
...
static void avr_acomp_reset( avr_io_t* port )
{
...
void avr_acomp_init( avr_t* avr, avr_acomp_t* p)
{
    p->io = _io;
