...
int avr_init( avr_t* avr )
{
    ...
    if( avr->custom.init) avr->custom.init(avr, avr->custom.data);
    if( avr->init) avr->init( avr );
...
{
    if( avr->custom.deinit ) avr->custom.deinit( avr, avr->custom.data );
...
void avr_reset( avr_t* avr )
{
    ...
    for( int i=0x20; i<=avr->ioend; i++) avr->data[i] = 0;
...
static void _avr_io_console_write(
        struct avr_t * avr,
        avr_io_addr_t addr,
        uint8_t v,
        void * param)
...
{
    if( addr ) avr_register_io_write( avr, addr, _avr_io_console_write, NULL );
}
