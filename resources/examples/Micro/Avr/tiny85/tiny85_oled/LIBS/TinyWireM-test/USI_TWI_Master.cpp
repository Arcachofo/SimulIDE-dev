                                                  ...
                                                  unsigned char msgSize)
{
  ...
  if (!(*msg & (1 << TWI_READ_BIT))) // The LSB in the address byte determines if is a
  ...
  do {
    ...
    if (USI_TWI_state.addressMode || USI_TWI_state.masterWriteDataMode)
    {
      ...
      } else USI_TWI_state.addressMode = FALSE; // Only perform address transmission once.
    }
...
unsigned char USI_TWI_Master_Start(void)
{
  PORT_USI |= (1 << PIN_USI_SCL); // Release SCL. /* Release SCL to ensure that (repeated) Start can be performed */
